...
load("Water_LADD.RData")
load("Diet_LADD.RData")
load("Dermal_LADD.RData")
...
for(food in names(LADD_sim)){
  
  for(chem in names(LADD_sim[[food]])){
...
#========================================================
# Total cumulative LADD
#========================================================

calc_total_ladd <- function(
    registry,
    group,
    chemical
){
...
for(group in names(EXPOSURE_REGISTRY)){
  ...
  TOTAL_LADD[[group]] <- list()
  
  for(chem in ALL_CHEMICALS){
    
    TOTAL_LADD[[group]][[chem]] <-
      tryCatch(
...
for(food in names(LADD_sim)){
  for(chem in names(LADD_sim[[food]])){
    ...
    tmp <- data.frame(
      ...
      Mean = c(
        mean(LADD_sim[[food]][[chem]]$m),
        mean(LADD_sim[[food]][[chem]]$f),
        mean(LADD_sim[[food]][[chem]]$t)
      ),
      P95 = c(
        unname(quantile(LADD_sim[[food]][[chem]]$m, 0.95)),
        unname(quantile(LADD_sim[[food]][[chem]]$f, 0.95)),
        unname(quantile(LADD_sim[[food]][[chem]]$t, 0.95))
      )
      )
...
for(sex in names(TOTAL_LADD)){
  
  HQ[[sex]] <- list()
  
  for(chem in names(TOTAL_LADD[[sex]])){
    
    if(!chem %in% names(TOX)) next
    
    ladd <- TOTAL_LADD[[sex]][[chem]]
    
    if(is.null(ladd) || length(ladd) == 0) next
    
    HQ[[sex]][[chem]] <- ladd / TOX[[chem]]
  }
}
...
#========================================================
# Pathway Contribution (clean + publication style)
#========================================================

contri_df_list <- list()

for(group in names(TOTAL_CONTRIBUTION)){
  for(chem in names(TOTAL_CONTRIBUTION[[group]])){
    
    tmp <- TOTAL_CONTRIBUTION[[group]][[chem]]
    if(is.null(tmp)) next
    
    tmp$Group <- group
    tmp$Chemical <- chem
    
    contri_df_list[[paste(group, chem)]] <- tmp
  }
}

contri_df <- dplyr::bind_rows(contri_df_list)

#========================================================
# Keep complete chemicals
#========================================================

total_pathway <- contri_df %>%
  distinct(Pathway) %>%
  nrow()

common_chem <- contri_df %>%
  group_by(Chemical) %>%
  summarise(pathway_n = n_distinct(Pathway), .groups = "drop") %>%
  filter(pathway_n == total_pathway) %>%
  pull(Chemical)

contri_df <- contri_df %>%
  filter(Chemical %in% common_chem)

#========================================================
# Labels (max per Pathway × Group × Chemical)
#========================================================

label_df <- contri_df %>%
  group_by(Group, Chemical, Pathway) %>%
  slice_max(Contribution, n = 1, with_ties = FALSE) %>%
  ungroup()

#========================================================
# Pathway order (high → low, left → right)
#========================================================

pathway_order <- contri_df %>%
  group_by(Pathway) %>%
  summarise(m = mean(Contribution, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(m)) %>%
  pull(Pathway)
...
#========================================================
# Figure 3: Exposure Pathway Contribution
#========================================================
p3 <- ggplot(
  contri_df,
  aes(x = Pathway, y = Contribution)
) +
  
  geom_col(width = 0.7, fill = "lightpink", color = "black") +
  
  facet_grid(Group ~ Chemical) +
  ...
  labs(
    title = "Exposure Pathway Contribution",
    x = NULL,
    y = "Contribution (%)"
  ) +
...
#========================================================
# Figure 3: PRCC Sensitivity Analysis
#========================================================
