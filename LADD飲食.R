...
get_food_summary <- function(food, chem){
  ...
  data.frame(
    Male = get_summary(LADD_sim[[food]][[chem]]$m),
    Female = get_summary(LADD_sim[[food]][[chem]]$f),
    Total = get_summary(LADD_sim[[food]][[chem]]$t)
  )
}
...
for(food in names(LADD_sim)){
  for(chem in names(LADD_sim[[food]])){
    ...
    tmp <- data.frame(
      ...
      P95 = c(
        quantile(LADD_sim[[food]][[chem]]$m,0.95),
        quantile(LADD_sim[[food]][[chem]]$f,0.95),
        quantile(LADD_sim[[food]][[chem]]$t,0.95)
      )
    )
...
write_xlsx(all_summary_df, "Diet_LADD_Summary.xlsx")
  ...
  for(chem in names(LADD_sim[[food]])){
    ...
    tmp <- rbind(
      data.frame(
        ...
        Chemical = chem,
        Group = "Male",
        LADD = LADD_sim[[food]][[chem]]$m
      ),
      data.frame(
        Food = food,
        Chemical = chem,
        Group = "Female",
        LADD = LADD_sim[[food]][[chem]]$f
      ),
      data.frame(
        Food = food,
        Chemical = chem,
        Group = "Total",
        LADD = LADD_sim[[food]][[chem]]$t
      )
    )
...
plot_canned_box <- function(df, title_text){
  ...
  ggplot(df, aes(x = Food, y = LADD, fill = Group)) +
