...
save(water_result_t, water_result_m, water_result_f, file = "Water_ADD.RData")
#========================================
...
plot_box <- function(result_list, group_name){
  ...
  df <- do.call(rbind, lapply(seq_along(result_list), function(i){
    data.frame(
      ...
      ADD = result_list[[i]]$ADD
    )
  }))
  
  # 正式標題
  plot_title <- paste("Drinking Water ADD Distribution –", group_name)
  
  # 箱型圖
  p <- ggplot(df, aes(x = Chemical, y = ADD)) +
...
make_tornado <- function(result_obj,
  ...
  sens <- data.frame(
    ...
    ADD = result_obj[[chem_name]]$ADD
  )
  
  # Spearman correlation(每個輸入變數 vs 最終 ADD 的相關性)
  tornado_df <- data.frame(
    Chemical    = chem_name,
    Group       = group_name,
    Variable    = c("Concentration", "Ingestion Rate", "Body Weight"),
    Correlation = c(
      cor(sens$C, sens$ADD, method="spearman"),
      cor(sens$IR, sens$ADD, method="spearman"),
      cor(sens$BW, sens$ADD, method="spearman")
    )
  )
...
write.csv(all_corr_df, "Water_ADD_Sensitivity.csv", row.names = FALSE)
...
make_summary <- function(x){
  ...
  data.frame(
    Chemical = names(x),
    Mean   = sapply(x, function(z) mean(z$ADD)),
    Median = sapply(x, function(z) median(z$ADD)),
