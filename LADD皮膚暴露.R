...
#========================
# LADD function (mg/kg/day)
#========================
calc_ladd <- function(Cpcp, AQ, EV, EF, ED, A, F, CF, BW, AT){
...
# BPA
#========================
LADD_BPA_m <- calc_ladd(C_BPA_shampoo, AQ$shampoo, EV$shampoo, EF, ED, A_BPA, F$shampoo, CF, BW_m, AT_m)
LADD_BPA_f <- calc_ladd(C_BPA_shampoo, AQ$shampoo, EV$shampoo, EF, ED, A_BPA, F$shampoo, CF, BW_f, AT_f)
LADD_BPA_t <- calc_ladd(C_BPA_shampoo, AQ$shampoo, EV$shampoo, EF, ED, A_BPA, F$shampoo, CF, BW_t, AT_t)
...
MeP <- function(BW, AT){
  list(
    shampoo = calc_ladd(C_MeP_shampoo, AQ$shampoo, EV$shampoo, EF, ED, A_MeP, F$shampoo, CF, BW, AT),
    perfume = calc_ladd(C_MeP_perfume, AQ$perfume, EV$perfume, EF, ED, A_MeP, F$perfume, CF, BW, AT),
    bodywash = calc_ladd(C_MeP_bodywash, AQ$bodywash, EV$bodywash, EF, ED, A_MeP, F$bodywash, CF, BW, AT),
    bodylotion = calc_ladd(C_MeP_bodylotion, AQ$bodylotion, EV$bodylotion, EF, ED, A_MeP, F$bodylotion, CF, BW, AT)
  )
}
...
#========================
dermal_result_t <- list(BPA_shampoo=LADD_BPA_t, MeP_shampoo=MeP_t$shampoo, MeP_perfume=MeP_t$perfume,
                 MeP_bodywash=MeP_t$bodywash, MeP_bodylotion=MeP_t$bodylotion)
dermal_result_m <- list(BPA_shampoo=LADD_BPA_m, MeP_shampoo=MeP_m$shampoo, MeP_perfume=MeP_m$perfume,
                 MeP_bodywash=MeP_m$bodywash, MeP_bodylotion=MeP_m$bodylotion)
dermal_result_f <- list(BPA_shampoo=LADD_BPA_f, MeP_shampoo=MeP_f$shampoo, MeP_perfume=MeP_f$perfume,
...
make_tornado <- function(result_obj, chem_name, group_name, BW){
  ...
  Correlation <- sapply(vars, function(v){
    if(length(unique(sens[[v]])) <= 1){
      ...
      return(cor(sens[[v]], sens$LADD, method="spearman"))
    }
  })
...
write.csv(all_corr_df, "Dermal_LADD_Sensitivity.csv", row.names=FALSE)
...
plot_dermal_as_water <- function(result_list, group_name){
  ...
  plot_title <- paste("Lifetime Average Daily Dose (LADD) via Dermal Exposure –", group_name)
  
  # 計算最大值，自動調整軸範圍
  max_LADD <- max(df$LADD, na.rm = TRUE)
  y_upper <- max_LADD * 1.1  # 留 10% 緩衝
  
  # 計算平均值與 P95，用於右上角圖示標註
  overall_mean <- mean(df$LADD, na.rm = TRUE)
  overall_p95  <- quantile(df$LADD, 0.95, na.rm = TRUE)
  
  # 使用箱型圖 + 平均值 + 離群值
  p <- ggplot(df, aes(x = Chemical, y = LADD, fill = Chemical)) +
