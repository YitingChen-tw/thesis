...
n_sim <- 10000
set.seed(123)
LT_m <- 76.94 * 365  # 男性平均餘命
LT_f <- 83.74 * 365  # 女性平均餘命  # 終身致癌評估分母 (年)
...
# 最後分別除以各自的平均餘命 LT
# 注意：分子已經包含了各階段的 (ED * 365)，所以分母直接除以 LT (年) 即可
inh_BPA_ladd_m <- pmax(total_daily_dose_m / LT_m, 0)
inh_BPA_ladd_f <- pmax(total_daily_dose_f / LT_f, 0)
...
# 建立資料框
sens_m <- data.frame(C  = C_sim, VE = VE_m_mean, BW = BW_m_mean, LADD = inh_BPA_ladd_m)
...
sens_f <- data.frame(C = C_sim, VE = VE_f_mean, BW = BW_f_mean, LADD = inh_BPA_ladd_f)
...
inh_BPA_ladd_t <- (inh_BPA_ladd_m + inh_BPA_ladd_f) / 2
...
save(
  inh_BPA_ladd_m, inh_BPA_ladd_f, inh_BPA_ladd_t,
  inh_C_BPA,
...
# 測試
plot_dist(m, "Male BPA Inhalation LADD Distribution",
          breaks_use = common_breaks, xmax_use = xmax_all)
plot_dist(f, "Female BPA Inhalation LADD Distribution",
          breaks_use = common_breaks, xmax_use = xmax_all)